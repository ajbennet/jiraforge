...
export const urlResponsePromiseCache = new LRUMap(URL_RESPONSE_CACHE_SIZE);
export default class CardClient {
  constructor(envKey, baseUrlOverride) {
    _defineProperty(this, "postBatchResolve", async (resources, resourceType = 'URL') => {
      var _Intl;
      ...
      try {
        ...
        if (resourceType === 'URL') {
          resolvedResources = await request('post', `${this.resolverUrl}/resolve/batch`, deDuplicatedResources.map(resourceUrl => ({
            resourceUrl
          })), headers);
        }
        // Ask the backend to resolve the ARIs for us.
        if (resourceType === 'ARI') {
          resolvedResources = await request('post', `${this.resolverUrl}/resolve/ari/batch`, deDuplicatedResources.map(ari => ({
            ari
          })), headers);
        }
      } catch (error) {
    ...
    _defineProperty(this, "batchResolveAris", async aris => {
  ...
  async resolveUrl(url, force = false) {
    ...
    const isUnresolvedLink = !isSuccessfulResponse(response) || getStatus(response.body) !== 'resolved';
    if (isUnresolvedLink) {
  ...
  async search(data) {
    ...
    const response = await request('post', `${this.resolverUrl}/invoke/search`, requestData);
  ...
  async fetchAvailableSearchProviders() {
    const response = await request('post', `${this.resolverUrl}/providers`, {
      type: 'search'
    });
    return response.providers;
  }
  mapErrorResponse(response, hostname = '') {
    ...
    response ? `${this.stringifyError(error)} ${JSON.stringify(rest)}` : 'Response undefined', 'UnexpectedError');
  }
