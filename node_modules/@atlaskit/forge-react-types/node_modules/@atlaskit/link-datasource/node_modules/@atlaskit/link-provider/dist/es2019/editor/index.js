...
import { getBaseUrl, getResolverUrl, request, getStatus } from '@atlaskit/linking-common';
...
const isJiraRoadmapOrTimeline = url => url.match(/^https:\/\/.*?\/jira\/software\/(c\/)?projects\/[^\/]+?\/boards\/.*?\/(timeline|roadmap)\/?/);
const isPolarisView = url => url.match(/^https:\/\/.*?\/jira\/polaris\/projects\/[^\/]+?\/ideas\/view\/\d+$|^https:\/\/.*?\/secure\/JiraProductDiscoveryAnonymous\.jspa\?hash=\w+|^https:\/\/.*?\/jira\/polaris\/share\/\w+|^https:\/\/.*?\/jira\/discovery\/share\/views\/[\w-]+(\?selectedIssue=[\w-]+&issueViewLayout=sidebar&issueViewSection=[\w-]+)?$/);
const isJwmView = url => url.match(/^https:\/\/.*?\/jira\/core\/projects\/[^\/]+?\/(timeline|calendar|list|board|summary|(form\/[^\/]+?))\/?/);
const isGiphyMedia = url => url.match(/^https:\/\/(.*?\.)?giphy\.com\/(gifs|media|clips)\//);
const isProformaView = url => url.match(/^https:\/\/[^/]+\/jira\/(core|software(\/c)?|servicedesk)\/projects\/\w+\/forms\/form\/direct\/\d+\/\d+.*$/);
...
export class EditorCardProvider {
  constructor(envKey, baseUrlOverride) {
  ...
  async fetchProvidersData() {
    const endpoint = `${this.resolverUrl}/providers`;
    const response = await request('post', endpoint, undefined, this.requestHeaders);
    return {
      patterns: response.providers.reduce((allSources, provider) => {
  ...
  async findUserPreference(url) {
    var _this$providersData;
    if (!this.providersData) {
      await this.loadProviderData();
    }
    const userPreferences = (_this$providersData = this.providersData) === null || _this$providersData === void 0 ? void 0 : _this$providersData.userPreferences;
    if (userPreferences) {
      const {
        defaultAppearance,
        appearances
      ...
      if (allMatchedLabeledAppearances.length > 0) {
        const longestMatchedLabeledAppearance = allMatchedLabeledAppearances.reduce((previousLabeledAppearance, currentLabeledAppearance) => previousLabeledAppearance.urlSegment.length > currentLabeledAppearance.urlSegment.length ? previousLabeledAppearance : currentLabeledAppearance);
  ...
  async findPatternData(url) {
    var _this$providersData2;
    if (!this.providersData) {
      await this.loadProviderData();
    }
    return (_this$providersData2 = this.providersData) === null || _this$providersData2 === void 0 ? void 0 : _this$providersData2.patterns.find(pattern => url.match(pattern.source));
  }
  getHardCodedAppearance(url) {
   ...
   * @private
   */
  async canBeResolvedAsEmbed(url) {
    try {
  ...
  async resolve(url, appearance, shouldForceAppearance) {
    try {
      ...
      const [matchedProviderPattern, userPreference] = await Promise.all([this.findPatternData(url), this.findUserPreference(url)]);
      if (shouldForceAppearance === false && userPreference === 'url') {
        return Promise.reject(undefined);
      }
      let isSupported = !!matchedProviderPattern || (await this.check(url));
      if (isSupported) {
        const providerDefaultAppearance = matchedProviderPattern === null || matchedProviderPattern === void 0 ? void 0 : matchedProviderPattern.defaultView;
        let preferredAppearance = shouldForceAppearance === undefined ?
      ...
      // eslint-disable-next-line
      console.warn(`Error when trying to check Smart Card url "${url}"${e instanceof Error ? ` - ${e.name} ${e.message}` : ''}`, e);
    }
    return Promise.reject(undefined);
  }
}
