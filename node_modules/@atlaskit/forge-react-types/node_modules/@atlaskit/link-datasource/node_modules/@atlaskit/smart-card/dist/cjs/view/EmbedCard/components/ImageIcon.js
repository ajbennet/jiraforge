"use strict";

var _interopRequireDefault = require("@babel/runtime/helpers/interopRequireDefault");
Object.defineProperty(exports, "__esModule", {
  value: true
});
exports.ImageIcon = void 0;
var _react = _interopRequireDefault(require("react"));
var _reactRenderImage = _interopRequireDefault(require("react-render-image"));
var _platformFeatureFlags = require("@atlaskit/platform-feature-flags");
var _styled = require("./styled");
var _styledOld = require("./styledOld");
var ImageIcon = exports.ImageIcon = function ImageIcon(_ref) {
  var _ref$alt = _ref.alt,
    alt = _ref$alt === void 0 ? '' : _ref$alt,
    src = _ref.src,
    _ref$size = _ref.size,
    size = _ref$size === void 0 ? 16 : _ref$size,
    title = _ref.title,
    defaultIcon = _ref.default;
  // TODO: do we need this?
  if (!src) {
    return defaultIcon || null;
  }
  var LoadedImageComponent = (0, _platformFeatureFlags.fg)('bandicoots-compiled-migration-smartcard') ? /*#__PURE__*/_react.default.createElement(_styled.Image
  // eslint-disable-next-line @atlaskit/ui-styling-standard/no-classname-prop -- Ignored via go/DSP-18766
  , {
    className: "smart-link-icon",
    src: src,
    alt: alt,
    size: size,
    title: title
  }) : /*#__PURE__*/_react.default.createElement(_styledOld.Image
  // eslint-disable-next-line @atlaskit/ui-styling-standard/no-classname-prop -- Ignored via go/DSP-18766
  , {
    className: "smart-link-icon",
    src: src,
    alt: alt,
    size: size,
    title: title
  });
  return /*#__PURE__*/_react.default.createElement(_reactRenderImage.default, {
    src: src,
    loading: defaultIcon,
    loaded: LoadedImageComponent,
    errored: defaultIcon
  });
};